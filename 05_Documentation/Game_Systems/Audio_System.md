...
| ambient_campfire_crackle | Player near a lit campfire (`FireManager`'s lit state, added 2026-09-25 with Fire Building) | Not part of the original 22 — added once Fire Building existed to trigger it. **Implemented 2026-09-25 (Claude Code):** Mike's sourced `campfire.wav` (originally 141s/96kHz/32-bit float stereo, 108 MB) is now on the Campfire prefab's AudioSource, imported mono/compressed (~1 MB at runtime) so it positions correctly in 3D — the original stays in LFS. Plays at volume 0.5 via a new Crackle Volume setting on the prefab, dropping as the fire dies down, and still follows the Ambient mixer slider. The earlier synthesized crackle is now only a fallback. |
...
| sfx_fish_bite | The bobber dipping ("plop") when a fish bites during active fishing | `fish-jumping-splash.wav`, volume 0.05 (down from 0.12). **Resolved 2026-09-25 (Claude Code):** the recording spends its first ~0.9s quietly building up before the real splash hits around 1s, then runs four more seconds of repeated splashing — that lead-in was why it felt slow. Now starts at 0.95s (the splash lands the instant the bobber dips), plays 10% faster, and stops after 0.75s with a quarter-second fade-out instead of running to the end. Volume dropped to 0.05 since the splash section reads about 3x louder than the clip's average, so it now sits level with the other sound effects. Made possible by new Max Duration/Fade Out/Pitch settings added to every sound on AudioManager (Bootstrap → Managers → AudioManager, per-sound in the Inspector) — Fish Bite is the only one using them so far, but they're available for any future sound that needs trimming or a pitch nudge. Confirmed in the editor sampler that it starts at 0.95s/1.1x speed and cuts off early rather than playing fully; the fade-out itself couldn't be directly sampled (checks run ~2s apart), so give it a listen. |
