...
- **Done and committed 2026-09-23 (Claude Code) — see Log.** Of the 3 items below, (2) and (3) are resolved: Git LFS is set up (audio, images/textures, 3D models incl. .blend, video, fonts — `.gitattributes`) and `Assets/Audio` is committed through it; `Xtra Sound files/` is gitignored. **Still open for Mike: (1) the two duplicate-file pairs need one replacement each** (no code change needed — drop in the new file). ~~**AudioManager implementation**~~ — all 22 Alpha 0.1 sounds are wired and verified in Play Mode: mixer with Music/Ambient/SFX/UI groups and per-channel PlayerPrefs volume, every documented trigger hooked up, a new `GroundSurface` component added as the surface-tag prerequisite the doc anticipated (placeholder ground defaults to Grass). Before Claude Code commits: (1) two sourced files turned out to be byte-for-byte duplicates of each other — `Ambient/Day.wav` = `Music/Gameplay Day.wav`, and `UI/menu button click.wav` = `UI/menu cancel click.wav` — Mike needs to source one replacement for each pair; (2) Git LFS isn't set up and `Assets/Audio` is ~180 MB — Mike needs to say whether to set up LFS first or commit the files directly; (3) an untracked `Xtra Sound files` folder at the repo root (confirmed by Claude to be the losing candidates from Mike's duplicate cleanup) needs a call — gitignore it or leave it alone.
