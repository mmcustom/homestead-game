...
- **Done 2026-09-23 (Claude Code) — see Log.** ~~**AudioManager implementation**~~ — all 22 Alpha 0.1 sounds wired and verified in Play Mode (mixer, Music/Ambient/SFX/UI groups, every documented trigger, `GroundSurface` prerequisite component added). Git LFS is set up and `Assets/Audio` committed through it; `Xtra Sound files/` is gitignored. Both duplicate-file pairs are now resolved: `UI/menu button click.wav`/`UI/menu cancel click.wav` — Mike confirmed reusing the same sound for both sfx_ui_click and sfx_ui_back, no replacement needed; `Ambient/Day.wav`/`Music/Gameplay Day.wav` — Mike replaced `Ambient/Day.wav`'s content with a new recording (same filename, so the Sourced Files mapping in Audio_System.md still holds). Nothing left open — Claude Code should verify `Day.wav` sounds distinct from `Gameplay Day.wav` in Play Mode and commit.
