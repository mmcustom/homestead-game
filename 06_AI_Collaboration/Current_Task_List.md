...
- **Partly resolved 2026-09-23 (Mike, batch review) — see Log.** **InventoryManager capacity defaults and item data** — capacity numbers confirmed and raised: encumbered above 30 kg (66 lbs), hard cap 45 kg (99 lbs), up from 25/40. Folded into Inventory_System.md. Claude Code still needs to update InventoryManager.cs and PlayerController.cs to match — see Needs Claude Code above. Item content is no longer open: `05_Documentation/Systems/Item_Data.md` now has a full draft item list (~41 items, weight/shelf-life grounded in the confirmed species docs) — ready for Claude Code to turn into real `ItemDefinition` assets, see Needs Claude Code above.
- **Drafted, ready for Claude Code — content creation, not a review question.** **DiscoveryManager and discovery content** — `05_Documentation/Systems/Discovery_Test_Sites.md` now has a draft set of 5 discoverable sites (one per Discovery_System.md category: Spring Hollow, Briar Patch, Old Fence Line Trail, Bass Hole, South Ridge Cabin Site), each with confirmed facts and rough relative placement. Explicitly a small test set, not the finished property layout — see Needs Claude Code above for what Claude Code should do with it.
...
Status: In Progress (3/3 identified Alpha 0.1 gaps drafted; 2 content-data docs added and drafted, pending Claude Code implementation and playtesting confirmation)
