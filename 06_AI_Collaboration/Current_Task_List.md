...
- **First Person Controller tuning (Claude Code, 2026-09-23)** — `Assets/Scripts/Player/PlayerController.cs` (Player prefab in World) implements First_Person_Controller.md. Placeholder defaults pending Mike's playtest: walk 2.5 m/s, sprint 5 m/s, crouch 1.3 m/s; speed drops to 50% at full encumbrance and sprinting is blocked when overloaded; stamina 100, sprint drains 15/s, recovers 25/s after 1 s; crouch height 1.1 m (standing 1.8 m, eyes 1.65 m); wildlife detection multipliers idle 0.7 / walk 1 / sprint 2 / crouch 0.4 for the future Wildlife/Hunting systems; interact range 2.5 m; discovery by line of sight up to 40 m (in addition to walking into a site). Sprint time is exposed for Core_Survival_System.md's Fatigue cost once that exists. Jumping and swimming are left out per Alpha 0.1 scope. World has a flat placeholder ground (200 m cube) until real terrain exists. `ItemPickup` is the first interactable; picked-up world objects aren't saved yet, so they reappear on reload.
- **Day/night cycle defaults (Claude Code, 2026-09-23)** — `Assets/Scripts/World/DayNightCycle.cs` drives the sun from TimeManager's per-season sunrise/sunset (east → south → west) with HDRP's Physically Based Sky (`Assets/Settings/WorldSkyAndFog.asset`, dynamic ambient, auto-exposure floor lowered to EV −2 so nights are dim but readable). Placeholder defaults pending Mike's review: midday sun elevation Spring/Fall 53°, Summer 76°, Winter 30° (~37°N latitude, blended across season boundaries); sunlight dimmed by weather (Cloudy 50%, Light Rain 35%, Heavy Rain 20%, Thunderstorm 15%, Snow 35%, Cold Front 80%, Wind 90%); cool 0.5 lux moonlight at night. Open: whether the property has a specific real-world latitude/region in mind, stars/moon phases, and cloud visuals for weather.
